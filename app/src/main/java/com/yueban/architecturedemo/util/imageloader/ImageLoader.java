...
public class ImageLoader {
  ...
  public static void displayImage(Context context, int resId, ImageView imageView, ImageDisplayOption option) {
    ...
    Picasso.with(context).load(resId).placeholder(option.getHolderDrawable(resources)).error(option.getErrorDrawable(resources))
        // TODO: 2017/7/20 最大尺寸设置根据屏幕宽度确定
        .resize(400, 0).into(imageView);
  }
  ...
  public static void displayImage(Context context, File file, ImageView imageView, ImageDisplayOption option) {
    Resources resources = context.getResources();
    Picasso.with(context).load(file).placeholder(option.getHolderDrawable(resources)).error(option.getErrorDrawable(resources))
