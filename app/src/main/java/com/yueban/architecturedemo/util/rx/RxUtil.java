...
public class RxUtil {
  ...
  public static <T> ObservableTransformer<T, T> ignoreError() {
    return new ObservableTransformer<T, T>() {
      @Override
      public ObservableSource<T> apply(@io.reactivex.annotations.NonNull Observable<T> upstream) {
        return upstream.onErrorResumeNext(Observable.<T>empty());
      }
    };
  }
   ...
   * 常用的组合
   */
  public static <T> ObservableTransformer<T, T> commonDialog(final IView view) {
    return new ObservableTransformer<T, T>() {
      @Override
      public ObservableSource<T> apply(@io.reactivex.annotations.NonNull Observable<T> upstream) {
        return upstream.compose(RxUtil.<T>applyAsySchedulers())
   ...
   * 常用的组合
   */
  public static <T> ObservableTransformer<T, T> commonWithError(final IView view) {
    return new ObservableTransformer<T, T>() {
      @Override
      public ObservableSource<T> apply(@io.reactivex.annotations.NonNull Observable<T> upstream) {
        return upstream.compose(RxUtil.<T>applyAsySchedulers())
   ...
   * 常用的组合
   */
  public static <T> ObservableTransformer<T, T> commonDialogWithError(final IView view) {
    return new ObservableTransformer<T, T>() {
      @Override
      public ObservableSource<T> apply(@io.reactivex.annotations.NonNull Observable<T> upstream) {
        return upstream.compose(RxUtil.<T>applyAsySchedulers())
